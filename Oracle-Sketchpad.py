...
def mainPage():
    cols = st.columns([6,4])
    with cols[0]:
        st.title('Oracle Sketchpad')
        st.header('What is Oracle Bone?')
        st.subheader('Oracle Bone Script originated over three thousand years ago during the Shang and Zhou periods in China, when characters were carved on ox scapulae and turtle shells for divination records. Essentially, it consists of visual symbols that ancient people "drew" using lines to represent nature and everyday life. Each oracle bone character resembles a miniature sketch, bridging visual art and language symbols.')
        st.header('What is 12 Zodiac Animals?')
        st.subheader('Rat, Ox, Tiger, Rabbit, Dragon, Snake, Horse, Goat, Monkey, Rooster, Dog, Pig,')
    with cols[1]:
...
def main():
    tabs = ["Main", "Painting", "Message"]

    selected_tab = st.query_params.get("tab", "Main")

    total_columns = 7
    button_start_index = (total_columns - len(tabs)) // 2

    cols = st.columns(total_columns)

    for i, tab in enumerate(tabs):
        if cols[button_start_index + i].button(tab, key=f"tab_{tab}"):
    ...
    if selected_tab == "Main":
        mainPage()
    elif selected_tab == "Painting":
        paintingPage()
    elif selected_tab == "Message":
        messagePage()
